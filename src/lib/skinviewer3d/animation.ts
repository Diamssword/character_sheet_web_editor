...
export class IdleAnimation extends PlayerAnimation {
	protected animate(player: PlayerObject): void {
		...
		player.forLayers(sk => { sk.leftArm.rotation.z = Math.cos(t) * 0.03 + basicArmRotationZ });
		player.forLayers(sk => { sk.rightArm.rotation.z = Math.cos(t + Math.PI) * 0.03 - basicArmRotationZ });
...
export class WalkingAnimation extends PlayerAnimation {
	...
	protected animate(player: PlayerObject): void {
		...
		player.forLayers(skin => {
			
			// Leg swing
			skin.leftLeg.rotation.x = Math.sin(t) * 0.1;
			skin.rightLeg.rotation.x = Math.sin(t + Math.PI) * 0.1;

			// Arm swing
			skin.leftArm.rotation.x = Math.sin(t + Math.PI) * 0.3;
			skin.rightArm.rotation.x = Math.sin(t) * 0.3;
			const basicArmRotationZ = Math.PI * 0.02;
			...
			if (this.headBobbing) {
				...
				skin.head.rotation.y = Math.sin(t / 4) * 0.2;
				skin.head.rotation.x = Math.sin(t / 5) * 0.1;
			} else {
